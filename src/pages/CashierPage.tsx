...
export default function CashierPage() {
  ...
  const activeOrders = useMemo(
    () => orders
      .filter((o: Order) => o.status !== 'تم التسليم')
  ...
  const todayOrders = orders.filter((o: Order) => o.timestamp >= todayStart.getTime());
  const todayRevenue = todayOrders.reduce(
    (sum: number, o: Order) => sum + o.items.reduce((s: number, i: OrderItem) => s + i.price * i.quantity, 0) + o.deliveryFee,
    0
  );
  const pendingCount = activeOrders.filter((o: Order) => o.status === 'جديد' || o.status === 'قيد التحضير').length;
    ...
    <div className="min-h-screen bg-[#0a0e1a]">
      <header className="bg-[#111827] border-b border-[#1f2937] px-4 md:px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleLogout} className="text-[#6b7280] hover:text-[#ef4444] transition-colors">
            <LogOut size={18} />
          </button>
        </div>
      </header>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[#9ca3af] text-xs">
                        {order.items.reduce((s: number, i: OrderItem) => s + i.quantity, 0)} عنصر
                      </span>
                      <span className="text-[#d4a853] font-bold text-sm">
                        {(order.items.reduce((s: number, i: OrderItem) => s + i.price * i.quantity, 0) + order.deliveryFee).toLocaleString('ar-IQ')} د.ع
                      </span>
                    </div>
                  </motion.div>
                ))}
              ...
              {selectedOrder ? (
                ...
                >
                  ...
                  <div className="mb-5">
                    ...
                    <div className="border-t border-[#1f2937] mt-3 pt-3 space-y-1">
                      ...
                      <div className="flex justify-between text-[#d4a853] font-bold text-lg">
                        <span>المجموع</span>
                        <span>
                          {(selectedOrder.items.reduce((s: number, i: OrderItem) => s + i.price * i.quantity, 0) + selectedOrder.deliveryFee).toLocaleString('ar-IQ')} د.ع
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <button onClick={handlePrint} className="flex items-center justify-center gap-2 bg-[#d4a853] hover:bg-[#e8c068] text-[#0a0e1a] py-2.5 rounded-xl text-sm font-semibold transition-all">
                      <Printer size={14} />
                      <span>طباعة</span>
                    </button>
                    {selectedOrder.status === 'جديد' && (
                      <button onClick={() => handleStatusUpdate(selectedOrder.id, 'قيد التحضير')} className="flex items-center justify-center gap-2 bg-[#1a2235] hover:bg-[#3b82f6]/20 text-[#3b82f6] border border-[#3b82f6]/30 py-2.5 rounded-xl text-sm font-semibold transition-all">
                        <RotateCcw size={14} />
                        <span>تحضير</span>
                      </button>
                    )}
                    {selectedOrder.status === 'قيد التحضير' && (
                      <button onClick={() => handleStatusUpdate(selectedOrder.id, 'جاهز')} className="flex items-center justify-center gap-2 bg-[#1a2235] hover:bg-[#22c55e]/20 text-[#22c55e] border border-[#22c55e]/30 py-2.5 rounded-xl text-sm font-semibold transition-all">
                        <Check size={14} />
                        <span>جاهز</span>
                      </button>
                    )}
                    {selectedOrder.status === 'جاهز' && (
                      <button onClick={() => handleStatusUpdate(selectedOrder.id, 'تم التسليم')} className="flex items-center justify-center gap-2 bg-[#1a2235] hover:bg-[#6b7280]/20 text-[#6b7280] border border-[#6b7280]/30 py-2.5 rounded-xl text-sm font-semibold transition-all">
                        <HomeIcon size={14} />
                        <span>تسليم</span>
                      </button>
                    )}
                    {selectedOrder.status === 'تم التسليم' && (
                      <button disabled className="flex items-center justify-center gap-2 bg-[#1a2235] text-[#6b7280] py-2.5 rounded-xl text-sm font-semibold opacity-50">
                        <Check size={14} />
                        <span>تم التسليم</span>
                      </button>
                    )}
                  </div>
                  <button onClick={() => { deleteOrder(selectedOrder.id); setSelectedOrderId(null); showToast('تم حذف الطلب'); }} className="w-full flex items-center justify-center gap-2 text-[#ef4444] hover:bg-[rgba(239,68,68,0.1)] py-2 rounded-xl text-sm transition-all">
                    <Trash2 size={14} />
                    <span>حذف الطلب</span>
                  </button>
                </motion.div>
              ) : (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#111827] border border-[#1f2937] rounded-2xl p-8 text-center sticky top-4">
